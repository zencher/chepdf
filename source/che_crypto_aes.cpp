...
int32_t CryptoRijndael::init(Mode mode, Direction dir, const unsigned char * key, KeyLength keyLen, unsigned char * initVector)
{
	// Not initialized yet
	state_ = Invalid;
	...
	for (int32_t i = 0; i < uKeyLenInBytes; i++)
	{
		keyMatrix[i >> 2][i & 3] = key[i];
	}
	keySched(keyMatrix);
	if (direction_ == Decrypt)
	{
		keyEncToDec();
	}
	state_ = Valid;
...
int32_t CryptoRijndael::blockEncrypt(const unsigned char * input, int32_t inputLen, unsigned char * outBuffer)
{
	int32_t i, k, numBlocks;
	...
	case ECB:
		...
		break;
	case CBC:
		reinterpret_cast<uint32_t*>(block)[0] = reinterpret_cast<const uint32_t*>(init_vector_)[0] ^ reinterpret_cast<const uint32_t*>(input)[0];
		reinterpret_cast<uint32_t*>(block)[1] = reinterpret_cast<const uint32_t*>(init_vector_)[1] ^ reinterpret_cast<const uint32_t*>(input)[1];
		reinterpret_cast<uint32_t*>(block)[2] = reinterpret_cast<const uint32_t*>(init_vector_)[2] ^ reinterpret_cast<const uint32_t*>(input)[2];
		reinterpret_cast<uint32_t*>(block)[3] = reinterpret_cast<const uint32_t*>(init_vector_)[3] ^ reinterpret_cast<const uint32_t*>(input)[3];
		encrypt(block, outBuffer);
		input += 16;
		for (i = numBlocks - 1; i > 0; i--)
		{
			reinterpret_cast<uint32_t*>(block)[0] = reinterpret_cast<const uint32_t*>(outBuffer)[0] ^ reinterpret_cast<const uint32_t*>(input)[0];
			reinterpret_cast<uint32_t*>(block)[1] = reinterpret_cast<const uint32_t*>(outBuffer)[1] ^ reinterpret_cast<const uint32_t*>(input)[1];
			reinterpret_cast<uint32_t*>(block)[2] = reinterpret_cast<const uint32_t*>(outBuffer)[2] ^ reinterpret_cast<const uint32_t*>(input)[2];
			reinterpret_cast<uint32_t*>(block)[3] = reinterpret_cast<const uint32_t*>(outBuffer)[3] ^ reinterpret_cast<const uint32_t*>(input)[3];
			outBuffer += 16;
			encrypt(block, outBuffer);
			input += 16;
		}
	...
	case CFB1:
#if STRICT_ALIGN 
		memcpy(iv, init_vector_, 16);
#else  /* !STRICT_ALIGN */
		*reinterpret_cast<uint32_t*>(iv[0]) = *reinterpret_cast<const uint32_t*>((init_vector_));
		*reinterpret_cast<uint32_t*>(iv[1]) = *reinterpret_cast<const uint32_t*>((init_vector_ + 4));
		*reinterpret_cast<uint32_t*>(iv[2]) = *reinterpret_cast<const uint32_t*>((init_vector_ + 8));
		*reinterpret_cast<uint32_t*>(iv[3]) = *reinterpret_cast<const uint32_t*>((init_vector_ + 12));
#endif /* ?STRICT_ALIGN */
		for (i = numBlocks; i > 0; i--)
		{
			for (k = 0; k < 128; k++)
			{
				*reinterpret_cast<uint32_t*>(block) = *reinterpret_cast<const uint32_t*>(iv[0]);
				*reinterpret_cast<uint32_t*>((block + 4)) = *reinterpret_cast<const uint32_t*>(iv[1]);
				*reinterpret_cast<uint32_t*>((block + 8)) = *reinterpret_cast<const uint32_t*>(iv[2]);
				*reinterpret_cast<uint32_t*>((block + 12)) = *reinterpret_cast<const uint32_t*>(iv[3]);
				encrypt(block, block);
				outBuffer[k / 8] ^= (block[0] & 0x80) >> (k & 7);
				iv[0][0] = static_cast<uint8_t> ((iv[0][0] << 1) | (iv[0][1] >> 7));
				iv[0][1] = static_cast<uint8_t> ((iv[0][1] << 1) | (iv[0][2] >> 7));
				iv[0][2] = static_cast<uint8_t> ((iv[0][2] << 1) | (iv[0][3] >> 7));
				iv[0][3] = static_cast<uint8_t> ((iv[0][3] << 1) | (iv[1][0] >> 7));
				iv[1][0] = static_cast<uint8_t> ((iv[1][0] << 1) | (iv[1][1] >> 7));
				iv[1][1] = static_cast<uint8_t> ((iv[1][1] << 1) | (iv[1][2] >> 7));
				iv[1][2] = static_cast<uint8_t> ((iv[1][2] << 1) | (iv[1][3] >> 7));
				iv[1][3] = static_cast<uint8_t> ((iv[1][3] << 1) | (iv[2][0] >> 7));
				iv[2][0] = static_cast<uint8_t> ((iv[2][0] << 1) | (iv[2][1] >> 7));
				iv[2][1] = static_cast<uint8_t> ((iv[2][1] << 1) | (iv[2][2] >> 7));
				iv[2][2] = static_cast<uint8_t> ((iv[2][2] << 1) | (iv[2][3] >> 7));
				iv[2][3] = static_cast<uint8_t> ((iv[2][3] << 1) | (iv[3][0] >> 7));
				iv[3][0] = static_cast<uint8_t> ((iv[3][0] << 1) | (iv[3][1] >> 7));
				iv[3][1] = static_cast<uint8_t> ((iv[3][1] << 1) | (iv[3][2] >> 7));
				iv[3][2] = static_cast<uint8_t> ((iv[3][2] << 1) | (iv[3][3] >> 7));
				iv[3][3] = static_cast<uint8_t> (((iv[3][3] << 1) | (outBuffer[k / 8] >> (7 - (k & 7)))) & 1);
			}
		}
		break;
	default:
...
int32_t CryptoRijndael::padEncrypt(const uint8_t * input, int32_t inputOctets, uint8_t * outBuffer)
{
	int32_t i, numBlocks, padLen;
	uint8_t block[16], *iv;
	...
	case ECB:
		...
		memset(block + 16 - padLen, static_cast<int32_t>(padLen), padLen);
		encrypt(block, outBuffer);
		break;
	case CBC:
		iv = init_vector_;
		for (i = numBlocks; i > 0; i--)
		{
			reinterpret_cast<uint32_t*>(block)[0] = reinterpret_cast<const uint32_t*>(input)[0] ^ reinterpret_cast<const uint32_t*>(iv)[0];
			reinterpret_cast<uint32_t*>(block)[1] = reinterpret_cast<const uint32_t*>(input)[1] ^ reinterpret_cast<const uint32_t*>(iv)[1];
			reinterpret_cast<uint32_t*>(block)[2] = reinterpret_cast<const uint32_t*>(input)[2] ^ reinterpret_cast<const uint32_t*>(iv)[2];
			reinterpret_cast<uint32_t*>(block)[3] = reinterpret_cast<const uint32_t*>(input)[3] ^ reinterpret_cast<const uint32_t*>(iv)[3];
			encrypt(block, outBuffer);
			iv = outBuffer;
			input += 16;
			outBuffer += 16;
		...
		{
			block[i] = static_cast<uint8_t>(input[i] ^ iv[i]);
		}
		for (i = 16 - padLen; i < 16; i++)
		{
			block[i] = static_cast<uint8_t>(static_cast<uint8_t>(padLen) ^ iv[i]);
		}
		encrypt(block, outBuffer);
		break;
	case CFB1:
...
int32_t CryptoRijndael::blockDecrypt(const uint8_t * input, int32_t inputLen, uint8_t * outBuffer)
{
	int32_t i, k, numBlocks;
	...
	if (state_ != Valid)
		return RIJNDAEL_NOT_INITIALIZED;
	if ((mode_ != CFB1) && (direction_ == Encrypt))
		return RIJNDAEL_BAD_DIRECTION;
	if (input == 0 || inputLen <= 0)
		return 0;
	...
	case CBC:
		...
		memcpy(iv,init_vector_,16);
#else
		*reinterpret_cast<uint32_t*>(iv[0]) = *reinterpret_cast<uint32_t*>((init_vector_));
		*reinterpret_cast<uint32_t*>(iv[1]) = *reinterpret_cast<uint32_t*>((init_vector_ + 4));
		*reinterpret_cast<uint32_t*>(iv[2]) = *reinterpret_cast<uint32_t*>((init_vector_ + 8));
		*reinterpret_cast<uint32_t*>(iv[3]) = *reinterpret_cast<uint32_t*>((init_vector_ + 12));
#endif
		for (i = numBlocks; i > 0; i--)
		{
			decrypt(input, block);
			reinterpret_cast<uint32_t*>(block)[0] ^= *reinterpret_cast<uint32_t*>(iv[0]);
			reinterpret_cast<uint32_t*>(block)[1] ^= *reinterpret_cast<uint32_t*>(iv[1]);
			reinterpret_cast<uint32_t*>(block)[2] ^= *reinterpret_cast<uint32_t*>(iv[2]);
			reinterpret_cast<uint32_t*>(block)[3] ^= *reinterpret_cast<uint32_t*>(iv[3]);
#if STRICT_ALIGN
			memcpy(iv, input, 16);
			memcpy(outBuf, block, 16);
#else
			*reinterpret_cast<uint32_t*>(iv[0]) = reinterpret_cast<const uint32_t*>(input)[0]; reinterpret_cast<uint32_t*>(outBuffer)[0] = reinterpret_cast<const uint32_t*>(block)[0];
			*reinterpret_cast<uint32_t*>(iv[1]) = reinterpret_cast<const uint32_t*>(input)[1]; reinterpret_cast<uint32_t*>(outBuffer)[1] = reinterpret_cast<const uint32_t*>(block)[1];
			*reinterpret_cast<uint32_t*>(iv[2]) = reinterpret_cast<const uint32_t*>(input)[2]; reinterpret_cast<uint32_t*>(outBuffer)[2] = reinterpret_cast<const uint32_t*>(block)[2];
			*reinterpret_cast<uint32_t*>(iv[3]) = reinterpret_cast<const uint32_t*>(input)[3]; reinterpret_cast<uint32_t*>(outBuffer)[3] = reinterpret_cast<const uint32_t*>(block)[3];
#endif
			input += 16;
			outBuffer += 16;
		}
	...
	case CFB1:
		...
		*reinterpret_cast<uint32_t*>(iv[3]) = *reinterpret_cast<uint32_t*>((init_vector_ + 12));
		...
		for (i = numBlocks; i > 0; i--)
		{
			for (k = 0; k < 128; k++)
			{
				*reinterpret_cast<int32_t*>(block) = *reinterpret_cast<const int32_t*>(iv[0]);
				*reinterpret_cast<int32_t*>((block + 4)) = *reinterpret_cast<const int32_t*>(iv[1]);
				*reinterpret_cast<int32_t*>((block + 8)) = *reinterpret_cast<const int32_t*>(iv[2]);
				*reinterpret_cast<int32_t*>((block + 12)) = *reinterpret_cast<const int32_t*>(iv[3]);
				encrypt(block, block);
				iv[0][0] = static_cast<uint8_t> ((iv[0][0] << 1) | (iv[0][1] >> 7));
				iv[0][1] = static_cast<uint8_t> ((iv[0][1] << 1) | (iv[0][2] >> 7));
				iv[0][2] = static_cast<uint8_t> ((iv[0][2] << 1) | (iv[0][3] >> 7));
				...
				iv[3][1] = static_cast<uint8_t> ((iv[3][1] << 1) | (iv[3][2] >> 7));
				iv[3][2] = static_cast<uint8_t> ((iv[3][2] << 1) | (iv[3][3] >> 7));
				iv[3][3] = static_cast<uint8_t> (((iv[3][3] << 1) | (input[k / 8] >> (7 - (k & 7)))) & 1);
				outBuffer[k / 8] ^= (block[0] & 0x80) >> (k & 7);
			}
		}
		break;
...
int32_t CryptoRijndael::padDecrypt(const uint8_t * input, int32_t inputOctets, unsigned char * outBuffer)
{
	int32_t i, numBlocks, padLen;
	uint8_t block[16];
	int32_t iv[4];
	...
	case CBC:
		...
		{
			decrypt(input, block);
			reinterpret_cast<uint32_t*>(block)[0] ^= iv[0];
			reinterpret_cast<uint32_t*>(block)[1] ^= iv[1];
			reinterpret_cast<uint32_t*>(block)[2] ^= iv[2];
			reinterpret_cast<uint32_t*>(block)[3] ^= iv[3];
			memcpy(iv, input, 16);
...
void CryptoRijndael::keySched(uint8_t key[_MAX_KEY_COLUMNS][4])
{
	uint32_t j, rconpointer = 0;

	// Calculate the necessary round keys
	// The number of calculations depends on keyBits and blockBits
	uint32_t uKeyColumns = rounds_ - 6;
	uint8_t tempKey[_MAX_KEY_COLUMNS][4];

	// Copy the input key to the temporary key matrix
	for (j = 0; j < uKeyColumns; j++)
	{
		*reinterpret_cast<uint32_t*>((tempKey[j])) = *reinterpret_cast<uint32_t*>((key[j]));
	}

	uint32_t r = 0;
	int t = 0;
	// copy values into round key array
	for (uint32_t r = 0; (j < uKeyColumns) && (r <= rounds_);)
	{
		for (; (j < uKeyColumns) && (t < 4); j++, t++)
		{
			*reinterpret_cast<uint32_t*>(expanded_key_[r][t]) = *reinterpret_cast<uint32_t*>(tempKey[j]);
		}
		if (t == 4)
		{
			r++;
	...
	while (r <= rounds_)
	{
		...
		if (uKeyColumns != 8)
		{
			...
			{
				*reinterpret_cast<uint32_t*>(tempKey[j]) ^= *reinterpret_cast<uint32_t*>(tempKey[j - 1]);
			}
		}
		else {
		...
		for (j = 0; (j < uKeyColumns) && (r <= rounds_);)
		{
			for (; (j < uKeyColumns) && (t < 4); j++, t++)
			{
				*reinterpret_cast<uint32_t*>(expanded_key_[r][t]) = *reinterpret_cast<uint32_t*>(tempKey[j]);
...
void CryptoRijndael::keyEncToDec()
{
	uint8_t * w = nullptr;
	for (uint32_t r = 1; r < rounds_; r++)
	{
		w = expanded_key_[r][0];
		*reinterpret_cast<uint32_t*>(w) = *reinterpret_cast<uint32_t*>(U1[w[0]]) ^ *reinterpret_cast<uint32_t*>(U2[w[1]])
			^ *reinterpret_cast<const uint32_t*>(U3[w[2]]) ^ *reinterpret_cast<const uint32_t*>(U4[w[3]]);
		w = expanded_key_[r][1];
		*reinterpret_cast<uint32_t*>(w) = *reinterpret_cast<uint32_t*>(U1[w[0]]) ^ *reinterpret_cast<uint32_t*>(U2[w[1]])
			^ *reinterpret_cast<const uint32_t*>(U3[w[2]]) ^ *reinterpret_cast<const uint32_t*>(U4[w[3]]);
		w = expanded_key_[r][2];
...
void CryptoRijndael::encrypt(const uint8_t a[16], uint8_t b[16])
{
	uint8_t temp[4][4];
	*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<const uint32_t*>((a)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[0][0]);
	*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<const uint32_t*>((a + 4)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[0][1]);
	*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<const uint32_t*>((a + 8)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[0][2]);
	*reinterpret_cast<uint32_t*>(temp[3]) = *reinterpret_cast<const uint32_t*>((a + 12)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[0][3]);
	*reinterpret_cast<uint32_t*>((b)) = *reinterpret_cast<const uint32_t*>(T1[temp[0][0]])
		^ *reinterpret_cast<uint32_t*>(T2[temp[1][1]])
		^ *reinterpret_cast<uint32_t*>(T3[temp[2][2]])
		^ *reinterpret_cast<uint32_t*>(T4[temp[3][3]]);
	*reinterpret_cast<uint32_t*>((b + 4)) = *reinterpret_cast<uint32_t*>(T1[temp[1][0]])
		^ *reinterpret_cast<uint32_t*>(T2[temp[2][1]])
		^ *reinterpret_cast<uint32_t*>(T3[temp[3][2]])
		^ *reinterpret_cast<uint32_t*>(T4[temp[0][3]]);
	*reinterpret_cast<uint32_t*>((b + 8)) = *reinterpret_cast<uint32_t*>(T1[temp[2][0]])
		^ *reinterpret_cast<uint32_t*>(T2[temp[3][1]])
		^ *reinterpret_cast<uint32_t*>(T3[temp[0][2]])
		^ *reinterpret_cast<uint32_t*>(T4[temp[1][3]]);
	*reinterpret_cast<uint32_t*>((b + 12)) = *reinterpret_cast<uint32_t*>(T1[temp[3][0]])
		^ *reinterpret_cast<uint32_t*>(T2[temp[0][1]])
		^ *reinterpret_cast<uint32_t*>(T3[temp[1][2]])
		^ *reinterpret_cast<uint32_t*>(T4[temp[2][3]]);
	for (uint32_t r = 1; r < rounds_ - 1; r++)
	{
		*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<uint32_t*>((b)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][0]);
		*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<uint32_t*>((b + 4)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][1]);
		*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<uint32_t*>((b + 8)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][2]);
		*reinterpret_cast<uint32_t*>(temp[3]) = *reinterpret_cast<uint32_t*>((b + 12)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][3]);
		*reinterpret_cast<uint32_t*>((b)) = *reinterpret_cast<uint32_t*>(T1[temp[0][0]])
			^ *reinterpret_cast<uint32_t*>(T2[temp[1][1]])
			^ *reinterpret_cast<uint32_t*>(T3[temp[2][2]])
			^ *reinterpret_cast<uint32_t*>(T4[temp[3][3]]);
		*reinterpret_cast<uint32_t*>((b + 4)) = *reinterpret_cast<uint32_t*>(T1[temp[1][0]])
	...
	*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<uint32_t*>((b)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[rounds_ - 1][0]);
	*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<uint32_t*>((b + 4)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[rounds_ - 1][1]);
	*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<uint32_t*>((b + 8)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[rounds_ - 1][2]);
	*reinterpret_cast<uint32_t*>(temp[3]) = *reinterpret_cast<uint32_t*>((b + 12)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[rounds_ - 1][3]);
	b[0] = T1[temp[0][0]][1];
	b[1] = T1[temp[1][1]][1];
	b[2] = T1[temp[2][2]][1];
	b[3] = T1[temp[3][3]][1];
	...
	b[14] = T1[temp[1][2]][1];
	b[15] = T1[temp[2][3]][1];
	*reinterpret_cast<uint32_t*>((b)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[rounds_][0]);
	*reinterpret_cast<uint32_t*>((b + 4)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[rounds_][1]);
	*reinterpret_cast<uint32_t*>((b + 8)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[rounds_][2]);
	*reinterpret_cast<uint32_t*>((b + 12)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[rounds_][3]);
}

void CryptoRijndael::decrypt(const uint8_t a[16], uint8_t b[16])
{
	uint8_t temp[4][4];
	*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<const uint32_t*>((a)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[rounds_][0]);
	*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<const uint32_t*>((a + 4)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[rounds_][1]);
	*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<const uint32_t*>((a + 8)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[rounds_][2]);
	*reinterpret_cast<uint32_t*>(temp[3]) = *reinterpret_cast<const uint32_t*>((a + 12)) ^ *reinterpret_cast<const uint32_t*>(expanded_key_[rounds_][3]);
	*reinterpret_cast<uint32_t*>((b)) = *reinterpret_cast<uint32_t*>(T5[temp[0][0]])
		^ *reinterpret_cast<uint32_t*>(T6[temp[3][1]])
		^ *reinterpret_cast<uint32_t*>(T7[temp[2][2]])
		^ *reinterpret_cast<uint32_t*>(T8[temp[1][3]]);
	*reinterpret_cast<uint32_t*>((b + 4)) = *reinterpret_cast<uint32_t*>(T5[temp[1][0]])
		^ *reinterpret_cast<uint32_t*>(T6[temp[0][1]])
		^ *reinterpret_cast<uint32_t*>(T7[temp[3][2]])
		^ *reinterpret_cast<uint32_t*>(T8[temp[2][3]]);
	*reinterpret_cast<uint32_t*>((b + 8)) = *reinterpret_cast<uint32_t*>(T5[temp[2][0]])
		^ *reinterpret_cast<uint32_t*>(T6[temp[1][1]])
		^ *reinterpret_cast<uint32_t*>(T7[temp[0][2]])
		^ *reinterpret_cast<uint32_t*>(T8[temp[3][3]]);
	*reinterpret_cast<uint32_t*>((b + 12)) = *reinterpret_cast<uint32_t*>(T5[temp[3][0]])
		^ *reinterpret_cast<uint32_t*>(T6[temp[2][1]])
		^ *reinterpret_cast<uint32_t*>(T7[temp[1][2]])
		^ *reinterpret_cast<uint32_t*>(T8[temp[0][3]]);
	for (uint32_t r = rounds_ - 1; r > 1; r--)
	{
		*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<uint32_t*>((b)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][0]);
		*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<uint32_t*>((b + 4)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][1]);
		*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<uint32_t*>((b + 8)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[r][2]);
	...
	*reinterpret_cast<uint32_t*>(temp[0]) = *reinterpret_cast<uint32_t*>((b)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[1][0]);
	*reinterpret_cast<uint32_t*>(temp[1]) = *reinterpret_cast<uint32_t*>((b + 4)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[1][1]);
	*reinterpret_cast<uint32_t*>(temp[2]) = *reinterpret_cast<uint32_t*>((b + 8)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[1][2]);
	*reinterpret_cast<uint32_t*>(temp[3]) = *reinterpret_cast<uint32_t*>((b + 12)) ^ *reinterpret_cast<uint32_t*>(expanded_key_[1][3]);
	b[0] = S5[temp[0][0]];
	b[1] = S5[temp[3][1]];
	b[2] = S5[temp[2][2]];
	b[3] = S5[temp[1][3]];
	...
	b[14] = S5[temp[1][2]];
	b[15] = S5[temp[0][3]];
	*reinterpret_cast<uint32_t*>((b)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[0][0]);
	*reinterpret_cast<uint32_t*>((b + 4)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[0][1]);
	*reinterpret_cast<uint32_t*>((b + 8)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[0][2]);
	*reinterpret_cast<uint32_t*>((b + 12)) ^= *reinterpret_cast<uint32_t*>(expanded_key_[0][3]);
}
